...
export default function EquityPage() {
  ...
  /* Admin-sign all non-member signature lines in the agreement HTML */
  const adminSignHtml = (html: string, memberName: string): string => {
    ...
    for (const match of Array.from(stakeholderBlocks)) {
      ...
      if (new RegExp(`^${escapedMember}$`, 'i').test(stakeholderName)) continue // skip member
  ...
  const sendAgreement = async () => {
    ...
    // Admin-sign all lines except the member's
